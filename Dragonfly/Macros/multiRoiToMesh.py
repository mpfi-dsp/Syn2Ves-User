...
for myMultiROI in listOfMultiROIs:
    ...
    for i in range(1, (_max + 1)):
        ...
        comX.append(((ROI.getCenterOfMass(guidROI, pTimeStep = 0))[0]) * 10**(6))
        comY.append(((ROI.getCenterOfMass(guidROI, pTimeStep = 0))[1]) * 10**(6))
        comZ.append(((ROI.getCenterOfMass(guidROI, pTimeStep = 0))[2]) * 10**(6))
        volume = (ROI.getVolume(guidROI, timeStep = 0)) * 10**(18)
        vol.append(volume)
        surfaceArea = ((ROI.getSurfaceFromWeightedVoxelEstimation(guidROI, timeStep = 0, progressBar = None))) * 10**(12) ### Linblad 2005 Method
    ...
    maxFeretLengthsA = multiRoiAnalyzer.getFeretLengthMax()
    minFeretLengthsA = multiRoiAnalyzer.getFeretLengthMin()

    maxFeretLength = [(10**(6))*i for i in maxFeretLengthsA[1:]]
    minFeretLength = [(10**(6))*i for i in minFeretLengthsA[1:]]
    ...
    array = np.column_stack((labels, comX, comY, comZ, vol, SA, halfSA, sphericity, maxFeretLength, minFeretLength, aspectRatio[1:]))
    df = pd.DataFrame(array, columns = ['labels', 'comX', 'comY', 'comZ', 'vol', 'SA', 'halfSA', 'sphericity', 'maxFeretLength', 'minFeretLength', 'aspectRatio'])
    
    measuresDir = (str(aFolder) + str('\\') + myMultiTitle + ' Measurements.csv').replace("\\","/")
    df.to_csv(measuresDir, encoding = 'utf-8-sig', index = False) 

    #with open(f'{aFolder}/meshExportRuntime.txt', 'w') as f:
